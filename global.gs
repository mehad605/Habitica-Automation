  ...
  try {
    ...
    if (webhookData.webhookType == "scored") {
      ...
      Object.assign(webhookData, {
        ...
        taskText: postData.task.text,
        taskNotes: postData.task.notes,
...
function processWebhook(webhookData) {
  ...
  if (webhookData.webhookType == "scored") {
    ...
    if (AUTO_SWITCH_EQUIPMENT === true && webhookData.taskType === "reward" && typeof webhookData.taskText !== "undefined") {
      scriptProperties.setProperty("switchEquipment", webhookData.taskText);
    }
    if (AUTO_BURST_OF_FLAMES === true && webhookData.taskText === "Burst of Flames" && webhookData.taskType === "reward") {
      scriptProperties.setProperty("castBurstOfFlames", webhookData.taskNotes || "0");
    }
  ...
  try {
    ...
    if (lock.tryLock(0) || (installing && lock.tryLock(360000))) {
      ...
      while (true) {
        ...
        let rewardName = properties["switchEquipment"];
        if (typeof rewardName !== "undefined") {
          switchEquipment(rewardName);
          scriptProperties.deleteProperty("switchEquipment");
          continue;
        }
        ...
        let bursts = properties["castBurstOfFlames"];
        if (typeof bursts !== "undefined") {
          castBurstOfFlames(bursts);
          scriptProperties.deleteProperty("castBurstOfFlames");
          continue;
        }
        break;
      }
 ...
 * getEquippedGear()
 * 
 * Fetches only the currently equipped gear from the Habitica API.
 * Uses userFields to minimize bandwidth and execution time.
 */
function getEquippedGear() {
  let response = fetch('https://habitica.com/api/v3/user?userFields=items.gear.equipped', GET_PARAMS);
  return JSON.parse(response).data.items.gear.equipped;
}

/**
 * fetchBatch(requests)
 * 
 * Parallelized wrapper for UrlFetchApp.fetchAll(requests).
 * Respects framework headers and rate-limiting data.
 */
function fetchBatch(requests) {
  ...
  // update rate limiting data from the last response in the batch
  if (responses.length > 0) {
    let lastResponse = responses[responses.length - 1];
    rateLimitRemaining = lastResponse.getHeaders()['x-ratelimit-remaining'];
    rateLimitReset = lastResponse.getHeaders()['x-ratelimit-reset'];
  }
